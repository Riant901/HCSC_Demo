...
node ('master') {
    stage('CONFIRM BUILD') {
        ...
        timeout(30) {
            input message: "You have selected the STAGE Deployment. Click proceed to initiate the QA Deployment?"
        }
    }
    stage('git Checkout') {
    ...
    stage('UAT Deployment') {
        ...
        cp -rf $ws/Stage_stage/output/*.war $ws/Stage/webapps/
        cp -rf $ws/Stage_stage/output/$phase.config $ws/Stage/configs/
